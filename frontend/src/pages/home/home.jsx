...
export default function Home() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get('http://localhost:8080/api/list-products');
    ...
    const handleCart = async (item) => {
        console.log(item);
        try {
            const response = await axios.post('http://localhost:8080/api/cart', { name: item });
        ...
        <>
            <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
                ...
                <Container sx={{ py: 8 }}>
                            ...
                            {products.map((item) => (
                                <Grid item xs={12} sm={6} md={3} key={item.id}>
                                    <Card sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
                                        <CardMedia
                                            component="div"
                                            sx={{ height: '200px', width: '100%', bgcolor: 'grey.200' }}
                                        >
                                            <img src={`http://localhost:8080/${item.img}`} alt={item.img} style={{ objectFit: "cover", height: '100%', width: "100%" }} />
                                        </CardMedia>
                                        <CardContent sx={{ flexGrow: 1 }}>
                                            ...
                                            <Typography gutterBottom variant="h6" component="h1" sx={{ fontSize: '13px' }}>
                                                {item.description}
                                            </Typography>

                                            <Box sx={{ display: "flex", justifyContent: "space-between" }}>
                                                <Typography gutterBottom variant="h6" component="h1" sx={{ fontSize: '14px' }}>
                                                    {item.ratings}
                                                </Typography>
                                                <Typography gutterBottom variant="h6" component="h1" sx={{ fontSize: '14px' }}>
                                                    {item.delivery_date}
                    ...
                    ) : (<Box sx={{ display: "flex", width: "100%", justifyContent: 'center', fontSize: "30px", marginTop: "5%", color: "blue" }}> No Products Found. Come after later</Box>)}
                </Container>
                ...
                <Dialog open={form} onClose={() => setForm(false)} fullWidth maxWidth="sm">
                    <Products handleFormClose={handleFormClose} handleSubmitted={handleSubmitted} />
                </Dialog>
            </Box>
            <ToastContainer
                position="top-right"
